...
function makeSourceMarkdown(levels) {
  const parts = [
...
function writeOutputs() {
  const correctedStageOneLevels = applyCourseContentOverrides(stageOneLevels);
  const correctedStageTwoLevels = applyCourseContentOverrides(stageTwoLevels);

  if (correctedStageOneLevels.length !== 50) {
    throw new Error(`Expected 50 stage one levels, found ${correctedStageOneLevels.length}`);
  }
  if (correctedStageTwoLevels.length !== 50) {
    throw new Error(`Expected 50 stage two levels, found ${correctedStageTwoLevels.length}`);
  }

  for (const level of correctedStageTwoLevels) {
  ...
  const combinedLevels = [...correctedStageOneLevels, ...correctedStageTwoLevels];
  ...
  writeFileSync(
    ...
    `// Generated by scripts/generate-textbook-stage2.mjs.\n` +
      `// Do not edit by hand.\n\n` +
      `export const stageTwoTextbookLevels = ${JSON.stringify(correctedStageTwoLevels, null, 2)};\n`
  );

  mkdirSync(dirname(sourcePath), { recursive: true });
  writeFileSync(sourcePath, makeSourceMarkdown(correctedStageTwoLevels));
  ...
  console.log(`Generated ${correctedStageTwoLevels.length} stage two levels and ${combinedLevels.length} total levels.`);
}
