...
更新时间：2026-07-01
项目路径：`/Users/linvwang/Documents/英语学习小程序`
...
- 最近验证：
  - `npm test` 通过，`94/94`
  - 线上 `index.html` 已确认加载 `src/app.mjs?v=stage3-assets-v5`
...
- 修复线上缓存未更新问题，将版本提升到 `stage3-assets-v5 / zh-hints-v5`。
- 新增插图重复审计脚本和报告。
- 新增 WebP 压缩脚本和测试。
...
最近一次审计检查了 4500 组题目图片，没有发现缺图。仍有一些 `semantic_review` 可疑项，很多属于“穿/脱、放入/拿出、相似动作”的视觉歧义，需要后续人工逐项确认或重新生图。

## 10. 当前风险与待办

- 1-100 关仍需继续抽查图片、英文、中文是否完全一致，特别是数量、位置、动作方向。
...
```text
请继续维护 `/Users/linvwang/Documents/英语学习小程序` 这个项目。先阅读 HANDOFF.md、README.md、README.zh-CN.md、package.json、src/app.mjs、src/game.mjs、src/hints.mjs。不要回退未提交改动。当前线上版本是 stage3-assets-v5 / zh-hints-v5，线上地址是 https://linc.wang/listen-pick/。项目是儿童英语听力选图学习系统，核心要求是英文语法、中文语义、插图逻辑必须严格正确，不能伤害儿童认知。优先处理图片/中文/英文不一致问题，然后再考虑 GitHub 推送、README 重写和小程序迁移。
```
